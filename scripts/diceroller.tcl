...
proc droll::pub_roll {nick userhost hand chan text} {
    set msg [droll::roll $chan [concat $nick " викинув"] $text]
...
proc droll::msg_roll {nick userhost hand text} {
    droll::roll $nick "Ти викинув" $text
    return 1
}
...
proc droll::roll {recipient msgPrefix text} {
	...
	if {$num > 1000} {
	    append msg " забагато кісток" 
	} else {
	    append msg " [droll::diceCalc $num $sides $mod]"
	}
    } else {
	append msg { невірно. Спробуй <n>d<x>[<mod>]: де \
                 <n> кількість кісток, \
                 <x> кількість сторін, \
                 <mod> вираз додавання або віднімання}
    }
