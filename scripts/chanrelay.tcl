...
namespace eval crelay {
	...
	set userlist(beg) "Начало списка пользователей"
	set userlist(end) "Конец списка пользователей"
...
namespace eval crelay {
    ...
    proc make:user { nick frm_bot } {
        ...
        if { $::crelay::me(snet) == "y" } {
            set speaker [concat "$::crelay::hlnick$nick$ehlnick"]
        } else {
    ...
    proc trans:who {nick uhost handle chan args} {
        if { [lindex [split $args] 1] != "" } {
            ...
            if { $him == -1 } {
                putserv "PRIVMSG $nick :$args est un rйseau inconnu";
                return 0
            } else {
    ...
    proc recv:nick {frm_bot command arg} {
        if {[set him [lsearch $::crelay::eggdrops $frm_bot]] >= 0} {
            set argl [split $arg]
            set speaker [[namespace current]::make:user [lindex $argl 0] $frm_bot]
            putquick "PRIVMSG $::crelay::me(chan) :*** $speaker теперь известен как [join [lrange $argl 1 end]]"
    ...
    proc recv:join {frm_bot command arg} {
        if {[set him [lsearch $::crelay::eggdrops $frm_bot]] >= 0} {
            ...
            putquick "PRIVMSG $::crelay::me(chan) :--> $speaker присоединился к каналу [lindex $argl 0]"
    ...
    proc recv:part {frm_bot command arg} {
        if {[set him [lsearch $::crelay::eggdrops $frm_bot]] >= 0} {
            set argl [split $arg]
            set speaker [[namespace current]::make:user [lindex $argl 0] $frm_bot]
            putquick "PRIVMSG $::crelay::me(chan) :<-- $speaker покинул канал [lindex $argl 1] ([join [lrange $argl 2 end]])"
    ...
    proc recv:quit {frm_bot command arg} {
        if {[set him [lsearch $::crelay::eggdrops $frm_bot]] >= 0} {
            set argl [split $arg]
            set speaker [[namespace current]::make:user [lindex $argl 0] $frm_bot]
            putquick "PRIVMSG $::crelay::me(chan) :-//- $speaker вышел ([join [lrange $argl 1 end]])"
    ...
    proc recv:topic {frm_bot command arg} {
        if {[set him [lsearch $::crelay::eggdrops $frm_bot]] >= 0} {
            set argl [split $arg]
            set speaker [[namespace current]::make:user [lindex $argl 0] $frm_bot]
#            putquick "PRIVMSG $::crelay::me(chan) :*** $speaker изменил топик [lindex $argl 1] на '[join [lrange $argl 2 end]]'"
        }
        return 0
    }
    ...
    proc recv:kick {frm_bot command arg} {
        if {[set him [lsearch $::crelay::eggdrops $frm_bot]] >= 0} {
            set argl [split $arg]
            set speaker [[namespace current]::make:user [lindex $argl 1] $frm_bot]
            putquick "PRIVMSG $::crelay::me(chan) :*** $speaker кикнут с [lindex $argl 2] [lindex $argl 0]: [join [lrange $argl 3 end]]"
    ...
    proc recv:mode {frm_bot command arg} {
        if {[set him [lsearch $::crelay::eggdrops $frm_bot]] >= 0} {
            set argl [split $arg]
            set speaker [[namespace current]::make:user [lindex $argl 1] $frm_bot]
#            putquick "PRIVMSG $::crelay::me(chan) :*** $speaker установил режим [join [lrange $argl 2 end]]"
        }
        return 0
    }
